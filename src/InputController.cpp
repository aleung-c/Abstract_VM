...
void				InputController::LexInput()
{
	...
	//std::cout << std::endl;
	//std::cout << " --------- Lexer: Program Tokenizing ------- " << std::endl;
	lineNumber = 0;
	numberInLine = 0;
...
void				InputController::ParseInput()
{
	...
	std::cout << "Program parsed -> OK for process" << std::endl;
	return ; // All green -> Go to processing;
}

void				InputController::TranslateInputToMachineInstruction()
{
...
void				InputController::ParseDictionnaryComparison(std::list<t_token>::iterator it, int *nbOfErrors)
{
	...
	if ((*it).TokenType == Instruction)
	{
		...
		if (find(VMSettings->InstructionsTable.begin(), VMSettings->InstructionsTable.end(), (*it).Value) == VMSettings->InstructionsTable.end())
		{
	...
	else if ((*it).TokenType == Value)
	{
		...
		valueType = std::strtok((char *)(*it).Value.c_str(), "(");
...
void				InputController::ParseSpecificRules(std::list<t_token>::iterator it, int *nbOfErrors)
{
	...
	if ((*it).TokenType == Instruction)
	{
		...
		/*if ( ((*it).Value == "push" || (*it).Value == "assert") // TODO: bug, a revoir.
			&& ((next == TokenList.end())
			|| (next != TokenList.end() && (*next).LineNumber != (*it).LineNumber)))
			nbOfErrors += 1;
			std::cout << "Line " << (*it).LineNumber << " col "
	...
	if ((*it).TokenType == Value)
	{
		...
		if (match.size() > 1) // TODO : bug aussi.
			valueString = match.str(0);
